def load_default_mapping():
    """Load your exact 266 trade mappings from the master data"""
    
    # Your exact 266 master trade mappings
    mapping_data_raw = """Room	Component	Trade
...
Upstairs Bathroom	Walls	Painting"""
    ...
    df = pd.read_csv(StringIO(mapping_data_raw), sep='\t')
    
    # Display actual count
    st.success(f"✅ Loaded exactly {len(df)} trade mappings from master data!")
    
    return df# Complete Working Streamlit App with Interactive Trade Mapping Management
