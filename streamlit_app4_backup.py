...
def main():
    ...
    if user['dashboard_type'] == 'inspector':
        show_secure_inspector_interface()
...
def show_secure_inspector_interface():
    """Secure inspector interface with proper permission checks"""
    ...
    if not secure_section_header(
        "Step 1: Load Master Trade Mapping", 
        "data.upload",
        "Upload your trade mapping file or use the default template"
    ):
        st.error("You need data upload permission to manage trade mappings")
        st.stop()
    ...
    with col2:
        # Template download (no permission needed)
...
Bedroom,Light Fixtures,Electrical"""
    ...
    # Secure file upload
    mapping_file = ui.secure_file_uploader(
        "Choose trade mapping CSV", 
        "data.upload",
        type=["csv"], 
        key="mapping_upload",
        disabled_message="You need data upload permission to upload mapping files"
    )
    ...
    with col1:
        if ui.secure_button("Load Master Mapping", "data.upload", type="secondary"):
            try:
    ...
    with col3:
        if ui.secure_button("Clear Mapping", "data.upload"):
            st.session_state.trade_mapping = pd.DataFrame(columns=["Room", "Component", "Trade"])
            st.session_state.step_completed["mapping"] = False
            st.rerun()
    
    # Display current mapping with secure dataframe
    if len(st.session_state.trade_mapping) > 0:
        ...
        ui.secure_dataframe(
            ...
            "data.view_assigned",
            height=200,
            disabled_message="You need data view permission to see the mapping"
        )
    else:
    ...
    if not secure_section_header(
        "Step 2: Upload Inspection Data", 
        "data.process",
        "Upload your inspection CSV file for processing"
    ):
        st.error("You need data processing permission to upload inspection data")
        st.stop()
    ...
    # Secure file upload for inspection data
    ...
    if uploaded_csv is not None:
        if ui.secure_button("Process Inspection Data", "data.process", type="primary", use_container_width=True):
            try:
                with st.spinner("Processing inspection data securely..."):
                    ...
                    # Use secure processing function
    ...
    if st.session_state.processed_data is not None and st.session_state.metrics is not None:
        if not secure_section_header(
            "Step 3: Analysis Results & Secure Downloads", 
            "data.view_assigned",
            "View processed data and generate secure reports"
        ):
            return
        
        show_secure_results_interface(ui, perm_manager, username)

def show_secure_results_interface(ui, perm_manager, username):
    """Show secure results interface with permission checks"""
    
    metrics = st.session_state.metrics
    
    # Building Information with access verification
    ...
    if not perm_manager.can_access_building(username, building_name):
        st.error(f"Access denied to building: {building_name}")
        return
    ...
    # Key Metrics Dashboard with permission check
    if perm_manager.has_permission(username, "data.view_assigned"):
        ...
        with col1:
            ui.secure_metric("Total Units", f"{metrics['total_units']:,}", 
                           "data.view_assigned", help="Total number of units inspected")
        
        with col2:
            ui.secure_metric("Total Defects", f"{metrics['total_defects']:,}", 
                           "data.view_assigned", delta=f"{metrics['defect_rate']:.1f}% rate")
        
        with col3:
            ui.secure_metric("Ready Units", f"{metrics['ready_units']}", 
                           "data.view_assigned", delta=f"{metrics['ready_pct']:.1f}%")
        
        with col4:
            ui.secure_metric("Avg Defects/Unit", f"{metrics['avg_defects_per_unit']:.1f}", 
                           "data.view_assigned", help="Average number of defects per unit")
        
        with col5:
            completion_efficiency = (metrics['ready_units'] / metrics['total_units'] * 100) if metrics['total_units'] > 0 else 0
            ui.secure_metric("Completion Efficiency", f"{completion_efficiency:.1f}%", 
                           "data.view_assigned", help="Percentage of units ready for handover")
    
    # Enhanced Unit Lookup with security
    st.markdown("---")
    if perm_manager.has_permission(username, "data.view_assigned"):
        st.markdown("""
        <div class="unit-lookup-container">
            <h3 style="text-align: center; margin-bottom: 1rem;">Secure Unit Defect Lookup</h3>
        ...
        with col2:
            ...
            if search_unit:
                try:
                    ...
                    # Log unit access
                    perm_manager.log_user_action(username, f"UNIT_LOOKUP: {search_unit}", 
                    ...
                    if len(unit_defects) > 0:
                        ...
                        # Detailed defect table with permission check
                        st.markdown("**Detailed Defect List:**")
                        ui.secure_dataframe(unit_defects, "data.view_assigned", use_container_width=True)
    ...
    # Summary Tables Section with secure tabs
    st.markdown("---")
    st.subheader("Summary Tables")
    
    # Define secure tabs configuration
    tab_configs = [
        {
            "name": "Trade Summary",
            "permission": "data.view_assigned",
            "content_func": lambda: show_trade_summary(ui, metrics)
        },
        {
            "name": "Unit Summary", 
            "permission": "data.view_assigned",
            "content_func": lambda: show_unit_summary(ui, metrics)
        },
        {
            "name": "Room Summary",
            "permission": "data.view_assigned", 
            "content_func": lambda: show_room_summary(ui, metrics)
        },
        {
            "name": "Urgent Defects",
            "permission": "data.view_assigned",
            "content_func": lambda: show_urgent_defects(ui, metrics)
        },
        {
            "name": "Planned Work",
            "permission": "data.view_assigned",
            "content_func": lambda: show_planned_work(ui, metrics)
        }
    ]
    
    ui.secure_tabs(tab_configs)
    
    # STEP 4: Generate & Download Secure Reports
    st.markdown("---")
    if secure_section_header(
        "Step 4: Generate & Download Secure Reports", 
        "reports.generate",
        "Create professional reports with audit logging"
    ):
        show_secure_report_generation(ui, perm_manager, username, metrics)

def show_trade_summary(ui, metrics):
    """Show trade summary with secure display"""
    st.markdown("**Trade-wise defect breakdown - Shows which trades have the most issues**")
    if len(metrics['summary_trade']) > 0:
        ui.secure_dataframe(metrics['summary_trade'], "data.view_assigned", use_container_width=True)
    else:
        st.info("No trade defects found")

def show_unit_summary(ui, metrics):
    """Show unit summary with secure display"""
    st.markdown("**Unit-wise defect breakdown - Shows which units need the most attention**")
    if len(metrics['summary_unit']) > 0:
        ui.secure_dataframe(metrics['summary_unit'], "data.view_assigned", use_container_width=True)
    else:
        st.info("No unit defects found")

def show_room_summary(ui, metrics):
    """Show room summary with secure display"""
    st.markdown("**Room-wise defect breakdown - Shows which room types have the most issues**")
    if len(metrics['summary_room']) > 0:
        ui.secure_dataframe(metrics['summary_room'], "data.view_assigned", use_container_width=True)
    else:
        st.info("No room defects found")

def show_urgent_defects(ui, metrics):
    """Show urgent defects with secure display"""
    ...
    if len(metrics['urgent_defects_table']) > 0:
        ...
        ui.secure_dataframe(urgent_display, "data.view_assigned", use_container_width=True)
        st.error(f"**{len(urgent_display)} URGENT defects require immediate attention!**")
    else:
        st.success("No urgent defects found!")

def show_planned_work(ui, metrics):
    """Show planned work with secure display"""
    ...
    with subtab1:
        ...
        if len(metrics['planned_work_2weeks_table']) > 0:
            ...
            ui.secure_dataframe(planned_2weeks, "data.view_assigned", use_container_width=True)
        else:
    ...
    with subtab2:
        ...
        if len(metrics['planned_work_month_table']) > 0:
            ...
            ui.secure_dataframe(planned_month, "data.view_assigned", use_container_width=True)
        else:
            st.success("No work planned for this period")

def show_secure_report_generation(ui, perm_manager, username, metrics):
    """Show secure report generation interface"""
    
    st.subheader("Complete Report Package")
    
    col1, col2 = st.columns(2)
    
    # Complete Package with security logging
    with col1:
        st.markdown("### Complete Secure Package")
        st.write("Excel + Word reports with audit trail")
        
        if ui.secure_button("Generate Complete Package", "reports.generate", type="primary", use_container_width=True):
            try:
                with st.spinner("Generating secure report package..."):
                    ...
                    st.success("Complete secure package generated!")
                    ui.secure_download_button(
    ...
    # Individual Reports with security
    with col2:
        st.markdown("### Individual Secure Reports")
        
        # Excel Report
        if ui.secure_button("Generate Excel Report", "reports.excel", type="secondary", use_container_width=True):
            try:
                with st.spinner("Generating secure Excel report..."):
                    if EXCEL_REPORT_AVAILABLE:
                        ...
                        st.success("Secure Excel report generated!")
                        ui.secure_download_button(
        ...
        # Word Report
        if WORD_REPORT_AVAILABLE:
            if ui.secure_button("Generate Word Report", "reports.word", type="secondary", use_container_width=True):
                try:
                    with st.spinner("Generating secure Word report..."):
                        ...
                        st.success("Secure Word report generated!")
                        ui.secure_download_button(
...
def show_secure_admin_interface():
    """Secure admin interface with enhanced permissions"""
    
    username = st.session_state.get("username")
    perm_manager = get_permission_manager()
    
    st.markdown("### Administrator Control Center")
    ...
    admin_mode = st.radio(
        "Select Admin Mode:",
        ["Data Processing", "System Administration", "Security Management"],
        index=0,
        horizontal=True,
        help="Choose between data processing, system management, or security oversight"
    )
    
    if admin_mode == "Data Processing":
        st.info("Full inspection processing interface with administrator privileges")
        show_secure_inspector_interface()  # Reuse inspector interface with admin privileges
    
    elif admin_mode == "System Administration":
        show_system_admin_interface()
    
    elif admin_mode == "Security Management":
        show_security_management_interface()
