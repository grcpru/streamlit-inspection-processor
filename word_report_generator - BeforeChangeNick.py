...
# Updated: Added dependency handling while preserving all original functionality
...
CHART_PALETTES = {
    ...
    'pastel_greens': ['#C1E8C1', '#9DD49D', '#7BC07B', '#5AAC5A', '#4A9C4A', '#3B8B3B', '#2C7C2C', '#1D6C1D'],
...
def create_enhanced_pie_chart(doc, metrics):
    ...
    # DEPENDENCY CHECK - If matplotlib not available, show text summary instead
    if not MATPLOTLIB_AVAILABLE:
        print("matplotlib not available - showing text summary instead of pie chart")
    ...
    try:
        ...
        if NUMPY_AVAILABLE and num_trades <= 12:
            ...
            base_colors = ['#8dd3c7', '#ffffb3', '#bebada', '#fb8072', '#80b1d3', 
                          '#fdb462', '#b3de69', '#fccde5', '#d9d9d9', '#bc80bd',
                          '#ccebc5', '#ffed6f', '#a6cee3', '#fdbf6f', '#e31a1c',
                          '#ff7f00', '#cab2d6', '#6a3d9a', '#b15928', '#ffff99']
...
def create_enhanced_severity_chart(doc, metrics):
    ...
    # DEPENDENCY CHECK - If matplotlib not available, show text summary instead
    if not MATPLOTLIB_AVAILABLE:
        print("matplotlib not available - showing text summary instead of severity chart")
...
def add_text_severity_summary(doc, metrics):
    ...
    try:
        ...
        if 'summary_unit' in metrics and len(metrics['summary_unit']) > 0:
            ...
            severity_data = [
                ("Critical (25+ defects)", critical_count),
                ("Extensive (15-24 defects)", extensive_count),
            ...
            for category, count in severity_data:
                if count > 0:
                    ...
                    severity_para.style = 'EnhancedBody'
...
def create_enhanced_trade_chart(doc, metrics):
    ...
    # DEPENDENCY CHECK - If matplotlib not available, skip chart
    if not MATPLOTLIB_AVAILABLE:
        print("matplotlib not available - skipping trade chart")
        return
    ...
    try:
        ...
        if NUMPY_AVAILABLE:
            ...
            # Fallback colors without numpy
            base_colors = ['#FFB3BA', '#BAFFC9', '#BAE1FF', '#FFFFBA', '#FFD1BA', 
                          '#E0BBE4', '#957DAD', '#D291BC', '#FEC8D8', '#FFDFD3']
            colors = base_colors[:len(top_trades)]
...
def create_enhanced_units_horizontal_chart(doc, metrics):
    ...
    # DEPENDENCY CHECK - If matplotlib not available, show text summary instead
    if not MATPLOTLIB_AVAILABLE:
        print("matplotlib not available - showing text summary instead of units chart")
...
def create_error_document(error, metrics):
    ...
    # Add dependency status information
    dep_header = doc.add_heading("Dependency Status", level=2)
    dep_text = f"""
Dependency Status at Time of Error:
- python-docx: Available
- matplotlib: {'Available' if MATPLOTLIB_AVAILABLE else 'Not Available'}
- seaborn: {'Available' if SEABORN_AVAILABLE else 'Not Available'}
- numpy: {'Available' if NUMPY_AVAILABLE else 'Not Available'}

If matplotlib is not available, charts will be replaced with text summaries.
This should not prevent the report from being generated.
    """
...
if __name__ == "__main__":
    print("Complete Word Report Generator with Dependency Handling loaded successfully!")
    print("\nDEPENDENCY STATUS:")
    ...
    print("\nKEY FEATURES:")
    print("• Complete functionality preserved from original")
    print("• Enhanced dependency handling with graceful fallbacks")
    print("• Text-based summaries when matplotlib unavailable")
    print("• Professional formatting maintained")
    print("• All original sections included:")
    print("  - Executive Overview")
    print("  - Inspection Process & Methodology") 
    print("  - Units Analysis with Charts")
    print("  - Defects Analysis")
    print("  - Data Visualization")
    print("  - Trade-Specific Summary with Tables")
    print("  - Component Breakdown Analysis")
    print("  - Strategic Recommendations")
    print("  - Comprehensive Footer")
    ...
    if not MATPLOTLIB_AVAILABLE:
        ...
        print("\nREADY: All dependencies available for full visual reports!")
