...
def show_corrected_inspector_interface():
    """Corrected inspector interface with proper permission granularity"""
    ...
    # STEP 1: Trade Mapping Section (show section, but check permissions for specific actions)
    st.markdown("""
...
Bedroom,Light Fixtures,Electrical"""
        ...
        # Simple template download - no permission check needed
        st.download_button(
            "Download Template",
    ...
    # File upload - CHECK PERMISSION HERE, not at section level
    ...
    with col2:
        # Master template download - NO PERMISSION NEEDED
        try:
            master_mapping = load_master_trade_mapping()
    ...
    # Display current mapping - CHECK PERMISSION for viewing data, not templates
    if len(st.session_state.trade_mapping) > 0:
    ...
    if perm_manager.has_permission(username, "data.upload"):
        ...
        if uploaded_csv is not None:
            # Show file preview (no permission needed for preview)
            try:
    ...
    # STEP 3: Results (show if data exists and user has view permission)
...
def show_secure_admin_interface():
    ...
    if admin_mode == "Data Processing":
        ...
        show_corrected_inspector_interface()  # Reuse inspector interface with admin privileges
