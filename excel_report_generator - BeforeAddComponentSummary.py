...
def generate_professional_excel_report(final_df, metrics):
    ...
    # === Core table formats (keep/update as needed) ===
    ...
    # Change num_format to 'dd/mm/yyyy' if you prefer.
    ...
    # A second header style used later (kept for compatibility)
...
def create_data_sheet(
    ...
    cell_format,
    alt_row_format,
    date_cell_format,        # pass a date format matching normal row style
    date_alt_row_format      # pass a date format matching alt row style
):
    """
    Create a data sheet with proper date cells AND preserve alternating-row styling.
    Reuses your existing header/cell/alt formats, and uses matching date formats
    for date columns so shading/borders stay identical.
    """
    import pandas as pd
    from datetime import datetime as _dt
...
def test_excel_generator():
    ...
    try:
        ...
        sample_data = pd.DataFrame({
            ...
            'PlannedCompletion': [45903, '2025-10-09']  # one Excel serial, one ISO date
        })
