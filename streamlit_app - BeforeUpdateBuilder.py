...
def show_simple_builder_interface_inline():
    """Main builder interface with inline work areas"""
    
    st.markdown("""
    <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
                color: white; padding: 1rem; border-radius: 10px; text-align: center; margin-bottom: 1rem;">
        <h2>Builder Workspace</h2>
        <p>Fix defects and upload completion photos</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    try:
        ...
        if not buildings_data:
            st.success("No defects to fix! All work is complete.")
            conn.close()
            return
        ...
        if selected_building_display:
            ...
            show_building_units_inline(cursor, selected_building_name, user)
...
if user['dashboard_type'] == 'admin':
    ...
    show_simple_builder_interface_inline()  # <-- NEW FUNCTION CALL
    st.stop()
